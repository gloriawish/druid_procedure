...
public class SQLEvalVisitorUtils {
    ...
    //add to eval expr use stack info
    public static Object evalExpr(String dbType,String expr,Stack<Map<String,Object>> stack, Object... parameters) {
        SQLExpr sqlExpr = SQLUtils.toSQLExpr(expr, dbType);
        return eval(dbType, sqlExpr,stack, parameters);
    }
    //add end
    ...
    //add to eval expr use stack info
    public static Object eval(String dbType, SQLObject sqlObject,Stack<Map<String,Object>> stack,Object... parameters) {
        Object value = eval(dbType, sqlObject,stack, Arrays.asList(parameters));
    ...
    //add to eval expr use stack info
    public static Object eval(String dbType, SQLObject sqlObject,Stack<Map<String,Object>> stack ,List<Object> parameters) {
        return eval(dbType, sqlObject, stack, parameters, true);
    }
    //add end
    
    //add to eval expr use stack info
    public static Object eval(String dbType, SQLObject sqlObject,Stack<Map<String,Object>> stack, List<Object> parameters, boolean throwError) {
        SQLEvalVisitor visitor = createEvalVisitor(dbType);
        visitor.setParameters(parameters);
        visitor.setVariableStack(stack);
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLBinaryOpExpr x) {
        ...
        switch (x.getOperator()) {
            ...
            case BooleanAnd:
            {
            	boolean left_boolean = eq(leftValue, true);
            	boolean right_boolean = eq(rightValue, true);
            	boolean result =left_boolean && right_boolean;
            	x.putAttribute(EVAL_VALUE, result);
            	break;
            }
            //add end
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLVariantRefExpr x) {
    	...
    	Stack<Map<String, Object>> stack=visitor.getVariableStack();
    	if(stack!=null)
    	{
	    	for (Map<String, Object> map : stack) {
				if(map.containsKey(x.getName()))
				{
		            ...
		            if (!containsValue) {
		                Object value =((VariableObject)map.get(x.getName())).getValue();
		                if (value == null) {
